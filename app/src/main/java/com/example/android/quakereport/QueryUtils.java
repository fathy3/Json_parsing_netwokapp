...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link earthquakcustomclass} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<earthquakcustomclass> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<earthquakcustomclass> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < featuresArray.length(); i++){
                JSONObject currentfutuer = featuresArray.getJSONObject(i);
                JSONObject propirtys = currentfutuer.getJSONObject("properties");
                double magnitude = propirtys.optDouble("mag");
                String place = propirtys.optString("place");
                long time = propirtys.getLong("time");
                String url = propirtys.optString("url");

                earthquakcustomclass earthquake = new earthquakcustomclass(magnitude, place, time, url);
